...
from model.request_models import Register, StatusEnum
...
# Configure logging for request_route
...
def get_manager_id(project_id: str, sql, cursor):
...
async def create_request(request: Register, current_user: dict = Depends(authenticate_user)):
    ...
    with get_db_connection() as (sql, cursor):
        ...
        if current_user["role"] != Role.manager or get_manager_id(request.projectId, sql, cursor) != current_user["username"]:
...
async def update_request(
    ...
    with get_db_connection() as (sql, cursor):
        ...
        if current_user["role"] != Role.manager or get_manager_id(projectId, sql, cursor) != current_user["username"]:
