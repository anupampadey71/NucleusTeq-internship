...
async def my_info(current_user: dict = Depends(authenticate_user)):
    ...
    if current_user["role"] not in [Role.admin, Role.manager, Role.user]:
        raise HTTPException(status_code=403, detail="Only admin or manager or employee can access employee info")
...
async def update_name(employeeId: str, name: str, current_user: dict = Depends(authenticate_user)):
    # Check if the user is admin
    if current_user["role"] != Role.admin:
